#!/bin/bash
# 口径对齐重跑：原来 T3 用 --max-pocket-atoms 256（当时为避 OOM），
# 而 T1 和其它所有模型用 511。6Å 口袋有 19.7% 超过 256 原子，
# 也就是说 HypSeek 在 T3 上有五分之一的靶点看到的是被随机裁剪的口袋，
...
LD_LIBRARY_PATH=$E/lib CUDA_VISIBLE_DEVICES=$GPU $E/bin/python ./unimol/test.py \
  ...
  --results-path "$B/results/t3_raw/hypseek_rk_511" \
  ...
  > "$B/results/logs/hypseek_T3_511.log" 2>&1
echo "hypseek 退出码=$?"
