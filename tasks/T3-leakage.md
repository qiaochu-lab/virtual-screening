...
- **B 组**（LigUnity ×2、LiTENCLIP、HypSeek）训了**同样这 16,744 个**，
  外加 2,196 个带 pAff 的靶点

**A 是 B 的真子集。** 这也让发现 13 的说法收窄了一圈：不是「训练数据比架构
重要」这种笼统说法，而是**同一批结构，加上亲和力标签之后，L1 的 EF@1% 从
17–19 涨到 32–39**。
...
组间均值差 **1.54 名**（B 组 −0.93，非 B 组 +0.62），Mann-Whitney U=1、
**p=0.0095**。
...
1. **「仅 P」格只有 7 个靶点。** 这是整个对比的功效瓶颈，结论是弱证据。
   第一版那个「完全分离、p=0.0048」在修正后变成「各有一个例外、p=0.0095」，
   B 组里 HypSeek 反向（+0.30）、非 B 组里 ConGLUDe 反向（−0.16）。
