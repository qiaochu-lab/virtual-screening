...
`ligand_novelty.py` 的 B 段（§3 的第二张表）用 `actives + decoys` 直接当分子
顺序，**只校验长度不校验标签**——正是本文档反复提到的那个坑。本脚本改成硬校验
（「标签为 1 的位置上确实是该靶点的 active」），代价是每层要跳过 3~18 个靶点，
表里的 ⚠️ 数字就是。§3 那张表没有重算，引用时注意。
