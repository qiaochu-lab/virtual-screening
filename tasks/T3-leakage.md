...
**一个没解决的循环性。** 新颖度档是按 **PocketAffDB 的配体**算的
（`train_label_blend_seq_full.json`），不是按各模型自己的训练配体。
所以这张表解决了靶点侧的循环性，配体侧的没解决——要解决得拿到每个模型的
训练配体清单，目前只有 SPRINT（139 万个 id）和 ConPLex（BindingDB 的
SMILES）拿得到。
