...
**一半可以排除，一半不能**（check_missed_vs_found_props.py，同样是那 5 个靶点）。
判据是**在同一批靶点上比 pooled 效应量和 paired 效应量**，不是比 p——
n=5 时 p 必然崩掉，Wilcoxon 双侧下界就是 2/2^5=0.0625。

                      pooled差    paired差   收缩
    MW                 +53.5      +13.0     76%   ← 靶点构成效应
    重原子                +3.0       +0.0    100%   ← 靶点构成效应
    可旋转键              -1.0       +0.0    100%   ← 靶点构成效应
    新颖度(亲和半)        -0.081     -0.075    8%   ← ⚠️ **没有收缩**

**尺寸和理化性质上排除了**：混合看差别大得吓人（MW p=3.3e-23），但那是检索失败
的靶点碰巧整体配体更大，同靶点内部分不开。

**新颖度上没有排除**：对 LigUnity-protein 自己的训练配体，效应量几乎不收缩——
同一靶点内部，检索漏掉的活性对训练化学确实更陌生约 0.075 Tanimoto，5 个里 4 个同向。
机制上几乎是同义反复，检索本来就是按「与训练化学像不像」排序的。

**所以主分析两个方向这样读（先写下来，再看数）：**

  · **missed ≈ found** → 物理不吃「对训练化学的熟悉度」这一套，确实能补上
    检索因化学不熟而漏掉的活性。**级联值得做的正面结果。**
  · **missed ≪ found** → **不能**说成「两组没有系统差异，所以只能是盲区重合」。
    能说的是：差异集中在对训练化学的相似度上，而 **Boltz-2 本身也是训练出来的
    模型**——两者共享的可能不是「口袋识别的盲区」，而是**对训练化学的依赖**。
    这比「盲区重合」更强，也和全文主线（模型靠化学系列记忆）是同一条。

⚠️ 这条控制自身的限制：能配对的靶点**按构造是检索部分成功的那批**。
检索完全失败的 3 个靶点（found 精确为 0）上，missed 活性有没有系统差异，测不了。
...
def main():
    ...
    if idx:
        ...
        print("  ⚠️ 解读前先看本文件开头：尺寸/理化性质的差异是靶点构成效应（已排除），")
        print("     但**新颖度那一维没有排除**——同靶点内 missed 对训练化学更陌生 0.075。")
